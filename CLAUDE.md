...
| `src/components/SlidesView.tsx` | Slide decks — **editable, AI-refinable, themeable** | Free for all (Pro gate removed). Renders **inline** — never auto-downloads. Backend `POST /tools/slides/{id}` returns **JSON `{slides,title}`**, not a blob; download is opt-in via `POST /tools/slides/{id}/download` and reuses the already-generated slides (**no second model call**). `SlideCanvas` uses container-query `cqw` units so one markup scales preview → fullscreen, and is theme-aware (`{preset, accent}` — Classic/Minimal/Bold + colour picker, mirrored in the .pptx). Edited **in place** via the inner `SlideEditor` (immutable array updates). This is the **one tool view whose `SectionComposer` input is fully wired** — typing an instruction calls `POST /tools/slides/{id}/refine`, **charged like a generation**. Cover author defaults `full_name` → `username` → "Guest" and is session-only (never persisted). ⚠️ **Ten layouts** (`type`: title/agenda/section/content/stats/comparison/process/cards/quote/closing) are drawn **twice** — HTML in `SlideCanvas`, .pptx in `backend/agents/slide_agent.py` — on one shared grid (100 × 56.25 units, 1 unit = 1cqw) with the same `palette()`. Change a layout's geometry or a theme colour in **both** files. The backend `_normalize_slide` coerces model output into valid layouts; the renderers trust that shape. |
...
| `src/components/PersonaModal.tsx` | View + change/update the active persona | Reachable from the navbar **Personalise** button (shows "Persona active" when set), open to **all registered users** (was Pro-only). Shows the **active persona text** at the top; the **Guided** tab is a one-click role picker (the shared 10 roles + General from `lib/roles.ts`) that calls `authApi.setPersonaRole(key)` to regenerate + apply instantly; the **Edit prompt** tab fine-tunes the text. Roles are shared with `RoleOnboarding` via `src/lib/roles.ts` (keys must match backend `ROLE_PRESETS`). |
...
- **Fonts** (loaded in `frontend/index.html`, mapped in `tailwind.config.js`): **Merriweather** is
  ...
  Don't add fonts without asking. **Inter** (loaded in `index.html`) is used **only inside slide previews**
  (`SlidesView` `SlideCanvas`, inline `fontFamily`) — don't remove its `<link>`. **Plus Jakarta Sans**
  is loaded but unused — see `STATUS.md` (an easy, untaken perf win).
