# Talktofile — Changelog

Dated log of meaningful changes, newest first. Moved out of `CLAUDE.md` on 2026-08-05
so the handover file stays small.

**One short line per meaningful change** — a feature landing, a decision, a revert. That's the whole
bar. Specifically:

- **Not** every file edit. **Skip this file entirely** for small tweaks, refactors, copy changes, and
  work-in-progress that might still move.
- **Never log churn.** If something was tried and reverted **in the same session**, it does not
  appear here at all — not the attempt, not the revert.
- **When unsure whether a change is meaningful, it isn't.** Update `STATUS.md` and skip this file.

Current state does **not** live here — that's `STATUS.md`, which is rewritten in place. Entries below
predate this convention and are longer than it allows; leave them as they are.
...
- **The UI never shows a raw email as a name.** New `frontend/src/lib/displayName.ts` returns
  ...
  The slide-deck cover author deliberately still uses the full address (user's call).
- **"Sage" finished being retired.** The persona generator no longer names the assistant, and
  migration `b8d3f1a95c27` rewrites personas already stored with the old name.

### 2026-08-05 — Mobile section dropdown, shorter composer placeholder, Beta badge dropped
**Done (frontend):**
- **Placeholder copy shortened** to `Add instructions (optional).` — in `Landing.tsx` (the hero chat
  box, non-chat modes; was "Add specific instructions here (optional).") and in `ChartsView.tsx`
  (was "Add specific instructions here."). The other tool views use "Add your preferences here." and
  were left alone.
- **Section tabs collapse to a dropdown below `sm`** (`ModeSwitcher.tsx`) — on a phone the 7 pills
  wrapped to ~3 rows and ate the section's vertical space. Below `sm` they're now one full-width
  trigger (active section's icon + label) that opens a menu of all 7; `sm`+ keeps the pill row
  untouched. Notes for the next session: the two are swapped with **CSS** (`sm:hidden` /
  `hidden sm:flex`), not a JS breakpoint, so **a change to one tab has to be made in both**; the menu
  opens **upward** (`bottom-full`) because the composer is bottom-anchored; it closes on
  outside-click/Escape; and the engaged cue survives the collapse as an orange row + dot per item,
  plus a dot on the closed trigger when another section has work in it. The now-dead mobile bits went
  with it — `SHORT_LABELS` ("Cards"/"Podcast") and the `sm:hidden`/`hidden sm:inline` label pair.
- **The "Beta" badge is gone from the whole app** — `grep -rE 'Beta|β' frontend/src` returns nothing.
  It used to sit on every non-Chat tab in `ModeSwitcher`. Over this session it was turned into `(β)`,
  restyled to inherit the label colour, superscripted, and added to the Landing hero tabs too —
  then **dropped entirely, by request: it cluttered the tab rows on both pages.** The short-lived
  `BetaBadge` export went with it. **Don't re-add it without asking** — this was a look-at-it-and-no
  decision, not an oversight. If it ever returns: a flex-item badge can't be raised with
  `align-super` (vertical-align is ignored on flex items) — `self-start` + `mt-px` is what worked.
- Type-check (`tsc --noEmit`) passes. ⚠️ **The dropdown has not been opened in a browser** — worth
  checking at 320/375px that it clears the composer and doesn't collide with the "Coming soon" bubble.

### 2026-08-05 — Intro-offer banner mobile fix, then parked it and the landing "Plans" section
**Done (frontend):**
- **Symptom:** in the `IntroOfferBanner` header, the "closes in" label collided with the Talktofile
  wordmark and the countdown ring on narrow screens — the header row (wordmark on the left, label +
  ring + close button on the right) has no room for all three below `sm`.
- **Fix:** the "closes in" span is now `hidden sm:inline` in `IntroOfferBanner.tsx`. Mobile shows just
  the countdown ring and the X; `sm`+ is unchanged. Type-check (`tsc --noEmit`) passes.

- **Then switched the banner off entirely** (same day, by request) — it's meant to return later, so
  nothing was deleted: `INTRO_OFFER_ENABLED = false` at the top of `App.tsx` short-circuits
  `scheduleIntroOffer` (so it can't burn the once-a-day localStorage slot), and the
  `<IntroOfferBanner>` render at the bottom of `App.tsx` is commented out. The component, its gating
  constants and the `signupNonce` wiring are all untouched. **To bring it back: flip the flag to
  `true` and un-comment the render — that's the whole change.**

- **Also hid the landing "Plans" section** ("Start free, upgrade anytime." + the Basic vs Pro
  comparison table) — same deal, coming back later. The whole `<section id="plans">` in
  `Landing.tsx` is commented out; `PLAN_FEATURES` stays at the top of the file. Checked first that
  nothing links to `#plans`, so no dead nav anchor was left behind. One wrinkle worth knowing:
  the section's inner `{/* Header row */}` / `{/* Feature rows */}` comments had to be un-braced,
  because a nested `*/` closes the outer JSX comment early — the restore note in-file says so.
- **Recorded both in `CLAUDE.md` under a new "Temporarily Hidden UI" section** — a table of what's
  off, where it lives, how it's off, and the exact steps to restore it, so neither gets deleted as
  dead code or rebuilt from scratch by a future session. `npm run build` (tsc + vite) passes.

**Pending / next:**
- **The mobile-header fix above is unverified visually** — the banner no longer renders, so check the
  header at 320/375px whenever the offer is switched back on.
- Landing was **not** re-checked for horizontal scroll at 320/375/768px after removing the Plans
  section — it only removed a block, so no reflow is expected, but it wasn't verified in a browser.
...
auto-detect headings→pages, chapter checklist + typed requests, build the shared mechanism + Summary
first, then extend to the other features):**
- **New primitive — chapter segmentation (`backend/agents/chapters.py`):** `segment_into_chapters(raw_text)`
  returns ordered `{index, id("ch1"…), title, start, end}` (char offsets into `raw_text`, so no text is
  duplicated). Heuristic (no model call): detect heading lines ("Chapter/Section/Part N", markdown `#`,
  conservative numbered "1. Title"; page markers excluded), split at them; fall back to `[Page N]`/`[Slide N]`
  ranges; else one "Full document" chapter. Substantial front-matter before the first heading becomes an
  "Introduction" chapter. Helpers: `public_chapters` (strip offsets for the API), `chapters_text` (concat
  selected chapters' text; empty/unknown ids → whole doc).
- **Stored + exposed:** `DocumentData.chapters` (populated in `orchestrator._analyse_into_document`) →
  `DocumentInfo.chapters` (`ChapterInfo{id,index,title}` in `schemas.py`) → included in the process-WS
  **`ready` event** and both `SessionInfo` builders (`routers/document.py`).
- **Scoped content:** `GET /document/{id}/content` now takes an optional `chapters=ch1,ch2` query param →
  returns only those chapters' text (backs the left panel's filtered view; omit → full document).
- **Scoped Summary endpoint (NEW):** `POST /tools/summary/{id}` `{filename?, chapter_ids?}`. No
  `chapter_ids` → returns the **precomputed** whole-doc summary (free, instant). With `chapter_ids` →
  summarises only those chapters via `generate_summary` (charged against the daily question limit). This
  is the **first real backend summary endpoint** (Summary was previously reveal-only).
- **Frontend shared pieces:** `lib/chapters.ts` (`parseChapterSelection` — parses "chapters 1 and 2",
  ranges "1-3", requires a chapter keyword, maps numbers to titles then position; `isWholeDocument`,
  `selectedTitles`); **`components/ChapterPicker.tsx`** (reusable chapter checklist with an "All chapters"
  = whole-doc option, rendered as a composer `pickerRow` — will be reused by the other features).
  `types` gained `Chapter` + `DocumentInfo.chapters`; `api/client.ts` gained `toolsApi.summary(...)` and a
  `chapterIds` arg on `documentApi.getContent`.
- **SummaryView:** shows the `ChapterPicker` (when >1 chapter), parses a typed request from its composer
  input (typed request wins over the checklist), and on generate either reveals the precomputed summary
  (whole doc) or fetches a chapter-scoped summary. A scoped result shows an amber "Summary of <chapters>
  only · Show full document" banner. It calls `onChapterScope(...)` so the left panel filters, and clears
  the scope on revert / unmount.
- **Left panel (`App.tsx` + `DocumentPanel.tsx`):** new `chapterScope` state; scoping opens the document
  panel on that file showing **only the selected chapters** (`DocumentPanel` fetches filtered content),
  with a banner + a **full/filtered toggle** and a "Clear filter" that returns to the whole document —
  "later it should go back and show full content" as requested. Scope resets on new session / reset / end.
- **Verified:** frontend `tsc --noEmit` passes; backend `import main` OK. Segmentation unit-tested
  (heading / page-fallback / single / empty). **Live end-to-end (real key):** uploaded a 3-chapter doc →
  chapters detected (`Chapter 1/2/3`), whole-doc summary reuses precomputed, **scoped summary of ch1+ch2**
  returns correctly (scope echoed), and `GET …/content?chapters=ch1,ch2` returns only chapters 1–2
  (excludes 3) while the unfiltered call includes all three.

**Pending / next:**
- **⚠️ Restart the backend** on deploy (new `/tools/summary` + `chapters` query param + pipeline change).
- **Extend chapter-scoping to the other features** (the agreed rollout): Translate, Flashcards, Podcast,
  Slides, Charts, and Chat. Each needs its tool endpoint to accept `chapter_ids` and scope the source text
  (Translate/Flashcards/Podcast/Slides are text-based like Summary; Charts is tabular; Chat needs
  chunk-level chapter tagging for retrieval — chunks aren't chapter-tagged yet, a follow-up). Reuse
  `ChapterPicker` + `parseChapterSelection` + the `onChapterScope`/left-panel wiring already built.
- **Frontend visual pass not done** (no browser driver): verify the picker, typed "chapters 1 and 2",
  the scoped-summary banner, and the left panel showing only those chapters + revert. Multi-doc scoping
  currently targets the **first** document only (note for the rollout). Check light/dark + 320–1280px.
...
theme presets + colour picker, author = full_name→username→"Guest"):**
- **1. Update the slides two ways.**
  - **Manual editing** — new "Edit slides" button opens an in-place `SlideEditor` (inner component in
    `SlidesView.tsx`): one editable card per slide (title, subtitle on the cover, per-bullet text
    inputs with add/remove, speaker note) + per-slide **move up/down / delete** and **"Add slide"**.
    All immutable array updates in `SlidesView`. "Done" returns to the deck view.
  - **AI refine** — the shared bottom `SectionComposer` input is now **wired** for slides
    (`value`/`onChange`/`onSubmit` → `refine()`); typing e.g. "add a slide on pricing" / "make slide 3
    shorter" and pressing Enter (or the **"Update slides"** button) calls the new backend refine route
    and swaps in the returned deck. A "Updating your slides…" spinner shows while in flight.
- **2. Cover credits the author.** `SlideCanvas` renders **"Created by {author}"** on the title slide;
  `author` defaults to `profile.full_name` → `username` → **"Guest"** (from `useAuth`) and is
  **editable** on the cover card in the editor. Passed through to the .pptx.
- **3. Designer layouts + colour.** New **theme picker row** above the composer: **Classic / Minimal /
  Bold** preset pills + a native `<input type="color">` accent picker (with Reset to `#E2611B`).
  `SlideCanvas` is now theme-aware (`{preset, accent}`) and mirrors each preset; the three presets are
  also implemented in the .pptx via `build_pptx(slides, title, theme, author)`.
- **Backend (`agents/slide_agent.py`):** added `refine_slides_data(documents, current_slides,
  instruction)` (LLM edits the existing deck, grounded on the doc; falls back to the current deck on a
  bad parse); rewrote `build_pptx` to accept `theme` (`preset` + `accent` hex, parsed via `_parse_accent`
  with brand-orange fallback) + `author`, split into `_build_title_slide` / `_build_content_slide` for
  the three presets. **Backend (`routers/tools.py`):** new `POST /tools/slides/{id}/refine`
  (`{slides, instruction}`, daily-limit charged like a generation); `SlidesDownloadRequest` extended with
  `theme` + `author`, threaded into `build_pptx`.
- **⚠️ Per-machine venv gap found:** `python-pptx==1.0.2` was declared in `requirements.txt` but **not
  installed in this machine's venv** (so the *existing* slides download would already 500 here — like the
  earlier youtube-transcript-api gap). Installed it. **The other dev may need `pip install -r
  requirements.txt`** if they hit a `No module named 'pptx'` on download.
- **Verified:** frontend `tsc --noEmit` passes; backend `import main` OK; `build_pptx` smoke-tested for
  all 3 presets + custom accent + author + bad-accent/bad-preset fallbacks (all produce valid .pptx bytes).
- **LIVE end-to-end verified (real OpenAI key)** via a scripted driver against the running backend:
  upload → process WS → ready → **generate** (8 slides) → **AI refine** ("add a Contact Us slide" →
  9 slides, correctly appended) → **themed download** (Bold preset, `#2563EB`, author) → the returned
  .pptx opens in python-pptx with 9 slides and the cover shows **"Created by Biswajith Gopinathan"**.
  ⚠️ **This machine's `.env` runs in Supabase auth mode** (`supabase_enabled=True`), so the API only
  accepts Supabase tokens — the driver minted an anonymous Supabase session to authenticate (legacy
  `/auth/guest` tokens are rejected here). Confirms the whole new backend path incl. the real LLM refine.

**Pending / next:**
- **Frontend visual pass still not done** (no browser driver available this session) — in the app:
  generate a deck → Edit slides (edit text, add/remove bullets, reorder, add/delete slide, set author) →
  switch presets + change colour (preview + fullscreen update) → AI refine → Download. The backend/LLM/
  .pptx path is confirmed live; only the React rendering (SlideCanvas presets, `SlideEditor` UI) is
  unverified visually. Check light/dark + 320–1280px.
- The edited author is session-only (not persisted to the profile) — by design.
